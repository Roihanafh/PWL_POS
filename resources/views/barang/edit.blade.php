    ...
    <div class="card card-outline card-primary">
        ...
        @else
            ...
            <div class="form-group row">
                ...
                <div class="col-10">
                    <input type="text" class="form-control" id="barang-nama" name="barang-nama" value="{{ old('barang-nama', $barang->{'barang-nama'}) }}" required>
                    @error('barang-nama')
                        <small class="form-text text-danger">{{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="col-10">
                    <select class="form-control" id="kategori_id" name="kategori_id" required>
                        ...
                        @foreach($kategori as $k)
                            <option value="{{ $k->kategori_id }}" {{ old('kategori_id', $barang->kategori_id) == $k->kategori_id ? 'selected' : '' }}>{{ $k->kategori_nama }}</option>
                        @endforeach
                    </select>
                    @error('kategori_id')
                        <small class="form-text text-danger">{{ $message }}</small>
